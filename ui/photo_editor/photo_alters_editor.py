...
from modules.photo_editor.photo_alters_engine import PhotoAltersEngine

from utils.sk_logger import sk_log


class PhotoAltersEditor(WorkerPhotoBase):

    MAX_LABEL_TEXT_LENGTH = 40

    def __init__(self, parent=None) -> None:
        try:
            super().__init__(editor_type="alter", parent=parent)
            with PhotoAltersEngine() as photo_alters_engine:
                try:
                    photo_alters_engine.alter_photo_cache_init(skip_check=True)
                except Exception as cache_error:
                    sk_log.warning(f"Cache initialization error: {cache_error}")
            self.initial_ui_setup()
        except Exception as e:
            sk_log.error(f"PhotoAltersEditor __init__ error: {e}")
            raise e
    ...
    def initial_ui_setup(self) -> None:
        ...
        try:
            with PhotoAltersEngine() as photo_alters_engine:
                try:
                    game_alters, local_workers = (
                        photo_alters_engine.fetch_alter_record_lists()
                    )
                except Exception as e:
                    sk_log.warning(
                        f"Cache fetch error, attempting rebuild: {e}"
                    )
                    photo_alters_engine.refresh_alter_photo_record_cache()
                ...
                self._populate_left_list(game_alters)
            ...
            sk_log.error(f"PhotoAltersEditor initial_ui_setup error: {e}")
            raise e

    def _populate_left_list(self, alter_list: List[dict]) -> None:
        """Populate the left list with the game alter names.

        Args:
            alter_list (List[dict]): The list of game alter names.
        ...
        try:
            if not alter_list:
                self.left_list.addItem("No game alters available")
                return
            for alter in alter_list:
                self.left_list.addItem(alter["game_alter_name"])
            self.left_list.sortItems()
        except Exception as e:
            sk_log.error(f"PhotoAltersEditor populate_left_list error: {e}")
            raise e

    def _populate_right_list(self, alter_list: List[dict]) -> None:
        """Populate the right list with the local alter photo files.

        Args:
            alter_list (List[dict]): The list of alter photo files.
        ...
        try:
            if not alter_list:
                ...
                return
            for filename in alter_list:
                self.right_list.addItem(filename["local_alter_photo_file"])
            self.right_list.sortItems()
        except Exception as e:
            sk_log.error(f"PhotoAltersEditor populate_right_list error: {e}")
            raise e
    ...
    def _left_list_item_toggled(self) -> None:
        ...
        try:
            ...
            if selected_item:
                ...
                filename, fileonly = self._fetch_photo_from_cache_by_alter_name(
                    alter_name
                )
                if filename:
                    ...
                    self.left_name_label.setText(alter_name)
            ...
            sk_log.error(
                f"PhotoAltersEditor left_list_item_selected error: {e}"
            )
            raise e
    ...
    def _left_side_reset(self) -> None:
        ...
        try:
            ...
            self.left_filename.setText("")
            self.unselect_left_button.setEnabled(False)
            self.unselect_left_button.setText("Unselect Alter")
        except Exception as e:
            sk_log.error(f"PhotoAltersEditor _left_side_reset error: {e}")
            raise e
    ...
    def _fetch_photo_from_cache_by_alter_name(
        self, alter_name: str
    ) -> Tuple[str, str]:
        """Fetch the photo from the cache by the alter name.

        Args:
            alter_name (str): The name of the alter.
        ...
        try:
            with PhotoAltersEngine() as photo_alters_engine:
                root_path = photo_alters_engine.worker_photo_path
                filename = photo_alters_engine.fetch_alter_filename_from_cache(
                    alter_name
                )
                if filename:
    ...
    def _fetch_photo_from_cache_by_filename(
        ...
        try:
            ...
            sk_log.error(
                f"PhotoAltersEditor _fetch_photo_from_cache_by_filename error: {e}"
            )
            raise e
    ...
    def _checkbox_state_changed(self) -> None:
        ...
        try:
            ...
            sk_log.error(
                f"PhotoAltersEditor _checkbox_state_changed error: {e}"
            )
            raise e
    ...
    def _text_input_changed(self) -> None:
        ...
        try:
            ...
            sk_log.error(f"PhotoAltersEditor _text_input_changed error: {e}")
            raise e
    ...
    def _clear_button_clicked(self) -> None:
        ...
        try:
            ...
            sk_log.error(f"PhotoAltersEditor _clear_button_clicked error: {e}")
            raise e
    ...
    def _transfer_up_button_clicked(self) -> None:
        ...
        try:
            with PhotoAltersEngine() as photo_alters_engine:
                ...
                photo_alters_engine.update_alter_photo_filename(
                    self.left_name_label.text(), filename_to_use, append_gif
                )
                self._left_list_item_toggled()
        except Exception as e:
            sk_log.error(
                f"PhotoAltersEditor _transfer_up_button_clicked error: {e}"
            )
            raise e
        ...
        try:
            ...
            with PhotoAltersEngine() as photo_alters_engine:
                try:
                    photo_alters_engine.refresh_alter_photo_record_cache()
                    game_alters, _ = (
                        photo_alters_engine.fetch_alter_photo_cache_lists()
                    )
                    self._populate_left_list(game_alters)
                except Exception as e:
                    sk_log.warning(f"Could not refresh game alters: {e}")
                    self._populate_left_list([])
            QMessageBox.information(
                self,
                "Refresh Complete",
                "Game alter list has been refreshed.",
            )
        except Exception as e:
            sk_log.error(f"PhotoAltersEditor _refresh_left_list error: {e}")
            QMessageBox.critical(
                self,
                "Error",
                f"An error occurred while refreshing game alter list: {str(e)}",
            )
            raise e
        ...
        try:
            ...
            with PhotoAltersEngine() as photo_alters_engine:
                try:
                    ...
                    self._populate_right_list(local_alters)
                except Exception as e:
                    sk_log.warning(f"Could not refresh full cache: {e}")
                    local_files = (
                        photo_alters_engine._fetch_alter_photos_from_dir()
                    )
                    local_alters = [
                        {"local_alter_photo_file": f} for f in local_files
                    ]
                    self._populate_right_list(local_alters)
            QMessageBox.information(
                self,
            ...
            sk_log.error(f"PhotoAltersEditor _refresh_right_list error: {e}")
            QMessageBox.critical(
                self,
                "Error",
    ...
    def _use_this_button_clicked(self) -> None:
        ...
        try:
            ...
            sk_log.error(
                f"PhotoAltersEditor _use_this_button_clicked error: {e}"
            )
            raise e
